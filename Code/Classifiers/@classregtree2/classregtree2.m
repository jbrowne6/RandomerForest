    ...
    if ~isfloat(X)
        ...
        if ~isempty(varnames)
                ...
                if ~iscellstr(varnames) || numel(varnames)~=nvars
                    error(message('stats:classregtree2:preparevars:BadNames', nvars));
                end
            else
                varnames = nvars;
            end
        ...
        if ~iscellstr(cnames) || nclass==0
            ...
            elseif isstruct(prior)
                if ~isfield(prior,'group') || ~isfield(prior,'prob')
                    error(message('stats:classregtree2:priorandcost:PriorStructMissingField'));
                end
                idx = getclassindex(cnames,prior.group);
                if any(idx==0)
                    j = find(idx==0);
                    error(message('stats:classregtree2:priorandcost:PriorStructMissingGroup', cnames{ j( 1 ) }));
                end
                prior = prior.prob(idx);
            end
            if ~isfloat(prior) || length(prior)~=nclass || any(prior<0) || all(prior==0)
                error(message('stats:classregtree2:priorandcost:BadPrior', nclass));
            end
            ...
            if isempty(cost)
                ...
                if isstruct(cost)
                    if ~isfield(cost,'group') || ~isfield(cost,'cost')
                        error(message('stats:classregtree2:priorandcost:CostStructMissingField'));
                    end
                    idx = getclassindex(cnames,cost.group);
                    if any(idx==0)
                        j = find(idx==0);
                        error(message('stats:classregtree2:priorandcost:CostStructMissingGroup', cnames{ j( 1 ) }));
                    end
                    cost = cost.cost(idx,idx);
                end
                if ~isequal(size(cost),nclass*ones(1,2))
                    error(message('stats:classregtree2:priorandcost:CostSizeMismatch', nclass, nclass));
                elseif any(diag(cost)~=0)
                    error(message('stats:classregtree2:priorandcost:CostNonzeroDiag'));
                elseif any(cost<0)
                    error(message('stats:classregtree2:priorandcost:CostNegative'));
            ...
            if nclass>1
                ...
                if sum(removerows)==length(id)
                    error(message('stats:classregtree2:priorandcost:ZeroCostForObservedClasses'));
                end
            end
        end
    end
...
error(message('stats:classregtree2:NoCatAllowed', upper( mfilename )));
end

function throwUndefinedError()
st = dbstack;
name = regexp(st(2).name,'\.','split');
error(message('stats:classregtree2:UndefinedFunction', name{ 2 }, mfilename));
end
...
for j=1:2:length(extra)
    if strncmp(extra{j},'catidx',length(extra{j}))
        ...
        error(message('stats:classregtree2:BadParamNameAndNotCatidx', extra{ j }));
    end
end
...
if skipchecks
    ...
    [X,Y,W,cnames] = classregtree2.preparedata(X,Y,W,doclass);
end
[N,nvars] = size(X);

% Process variable names
if ~skipchecks
    names = classregtree2.preparevars(names,nvars);
end
...
if doclass
    ...
    if skipchecks
        if ~isstruct(Prior) || ~isstruct(Cost)
            error(message('stats:classregtree2:BadPriorOrCostForSkipchecks'));
        end
        idx = getclassindex(cnames,Prior.group);
        Prior = Prior.prob(idx);
        idx = getclassindex(cnames,Cost.group);
        ...
        [Prior,Cost,removerows] = classregtree2.priorandcost(Prior,Cost,cnames,Wj,Y);
        if any(removerows)
            X(removerows,:) = [];
            C(removerows,:) = [];
            WC(removerows,:) = [];
...
if ~isempty(splitmin) && ~isempty(minparent)
    error(message('stats:classregtree2:BothSplitminAndMinparentNotAllowed'));
end
if ~isempty(splitmin)
    if ~isnumeric(splitmin) || ~isscalar(splitmin)
        error(message('stats:classregtree2:BadSplitmin'));
    end
    minparent = splitmin;
end
...
if minleaf<1
    error(message('stats:classregtree2:MinleafLessThanOne'));
end
if minparent<1
    error(message('stats:classregtree2:MinparentLessThanOne'));
end
minparent = max(minparent,2*minleaf);

% Compute surrogate splits?
if ~strcmpi(surrogate,'on') && ~strcmpi(surrogate,'off')
    error(message('stats:classregtree2:BadSurrogate'));
end
...
if ~success
    error(message('stats:classregtree2:BadNumberOfRandomFeatures', nvars));
end
nusevars = nvartosample;

% Check prune parameter
if ~strcmpi(Prune,'on') && ~strcmpi(Prune,'off')
    error(message('stats:classregtree2:BadPrune'));
end

% Check merge parameter
if ~strcmpi(Merge,'on') && ~strcmpi(Merge,'off')
    error(message('stats:classregtree2:BadMergeLeaves'));
end
