	...
	if ~iscell(Y)
		...
		if strcmp(ForestMethod,'rerf') && strcmp(RandomMatrix,'frc')
			...
			if rotate
				...
				%size.
				if i == 5
%					fprintf('samp sz is %d and number of samples is %d and oob size is %d\n',length(ibidx), length(sampleidx), length(oobidx{i}));
fprintf('this is ibidx- %d\n', ibidx);
%fprintf('this is oobidx- %d\n', oobidx);
end
